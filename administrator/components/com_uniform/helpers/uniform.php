	...
	public static function generateHTMLPages($formId, $formName, $formType = "", $topContent = "", $bottomContent = "", $showTitle = false, $showDes = false)
	{
		...
		if ($items)
		{
			...
			foreach ($getHeadData['scripts'] as $script => $option)
			{
				...
				{
					if (!empty($checkLoadJS['google-maps']) && empty($getHeadData['scripts']['https://ditu.google.cn/maps/api/js?sensor=false&libraries=places']))
					{
						if ($uri->getScheme() == 'https')
						{
							$scripts['https://ditu.google.cn/maps/api/js?sensor=false&libraries=places'] = $option;
						}
						else
						{
							$scripts['http://ditu.google.cn/maps/api/js?sensor=false&libraries=places'] = $option;
						}
			...
			if ($checkLoadScript)
			{
				...
				if (!empty($checkLoadJS['google-maps']))
				{
					if ($uri->getScheme() == 'https')
					{
						$document->addScript('https://ditu.google.cn/maps/api/js?sensor=false&libraries=places');
					}
					else
					{
						$document->addScript('http://ditu.google.cn/maps/api/js?sensor=false&libraries=places');
					}
